      ...
      UserModel.findOne({ googleId: profile.id }, async (err, doc) => {
        ...
        if (!doc) {
          const newUser = new UserModel({
            ...
            photo: photo: profile.photos[0].value,,
          });
          await newUser.save();
          return done(null, newUser); //if user is created and saved to databse pass that profile to deserializeUser for more consistentancy
        }
        return done(null, doc); //if user already exists in the database then pass that profile to deserializeuser
      });
    }
  )
);
...
passport.serializeUser((user, done) => {
  ...
  done(null, user._id); //from the database since we are passing currently made user or existing  user from database //safe practise to store only the id in session //the user.id is passed here and after this deserializeuser is run
});

passport.deserializeUser((id, done) => {
  //we get userid and later match the id with database and get the user object and store  in req.user
  UserModel.findById(id, (err, doc) => {
    return done(null, doc); //with id we deserialize the doc  and we now have whole userObj
  });
  // return done(null, id);
});
...
passport.use(
  new GitHubStrategy(
    ...
    function (accessToken, refreshToken, profile, done) {
      UserModel.findOne({ githubId: profile.id }, async (err, doc) => {
        if (err) {
          return done(err, null);
        }
        if (!doc) {
...
passport.use(
  new TwitterStrategy(
    ...
    function (token, tokenSecret, profile, done) {
      UserModel.findOne({ twitterId: profile.id }, async (err, doc) => {
        if (err) {
          return done(err, null);
        }
        if (!doc) {
